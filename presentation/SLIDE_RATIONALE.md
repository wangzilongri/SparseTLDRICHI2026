...
| Evidence | 07–12 | 7:15–15:00 | What was tested; finite-sample advantage; mechanism; robustness; real data; takeaways |
...
## Results-arc repair note

The current candidate deck is `output/transfer-learning-meta-analysis-ichi-2026-talk.results-arc.pptx`, generated by `repair_results_arc.mjs` from the previously repaired deck. This pass changed the results section because the old sequence was abrupt: it opened with aggregate ranks before the audience knew the regimes, baselines, or practical target-data setting.

The repaired evidence block is:

| Slide | Role | Rationale |
|-------|------|-----------|
| 07 | Evaluation setup | Define regimes, comparators, and metrics before showing numbers. |
| 08 | Connected finite-sample result | Start with the paper's clearest deployment case: scarce target labels and high dimension. |
| 09 | Mechanism / ablation logic | Explain why the gain is not generic pooling: target-placebo anchoring plus DR CATE learning. |
| 10 | Robustness and boundaries | Show source-scaling benefits and gradual degradation under A5 violations. |
| 11 | IHDP and disconnected caution | Confirm the pattern under real covariate shift while keeping disconnected claims transport-based. |

QA caution from this repair: do not hide stale text with white rectangles. Old titles and planning phrases can remain in the PPTX XML even when invisible in PNG previews. Delete stale text shapes before redrawing slide content and scan the PPTX internals for forbidden phrases and ASCII math fallbacks.
...
- The visible slide states the regime distinction directly: connected targets support identified CATE claims; disconnected targets support transport-based analysis under A6.
...
- Earlier drafts used a "What to say" label; that was removed because it read as speaker guidance on the slide face. The final wording keeps only audience-facing claims.
...
- Three cards map directly to the repaired results arc: regimes, comparators, and metrics before finite-sample results.
...
## Slide 08 — Connected targets: transfer helps when labels are scarce (8:30–10:00) · Transition in: cut 0 ms

Cut from slide 07: the finite-sample connected result is the first evidence delivery, directly following the evaluation setup. No pause warranted.

**Paper anchor**
- Table II (Dimension Sweep PEHE), lines 492-498. Three exact numbers at target budget (m0=150, m1=100):
  - p=20: TargetOnly 2.60 -> Proposed-CF 0.50
  - p=50: TargetOnly 4.75 -> Proposed 0.73
  - p=100: TargetOnly 7.57 -> Proposed 1.71

**Design rationale**
- The audience sees the practical deployment case before any summary ranks: small target trial, high dimension, shifted sources.
- Bar lengths emphasize the TargetOnly-to-Proposed gap without forcing the audience to parse Table II.
- This slide answers "when does transfer pay?" before asking the audience to trust an aggregate ranking.

Slide 08 makes the finite-sample claim. Slide 09 explains why the gap is not generic pooling.

---

## Slide 09 — Why the method helps: anchor, then learn CATEs (10:00–11:15) · Transition in: cut 0 ms

Cut: continuing the same synthetic evidence block, shifting from the observed performance gap to the mechanism behind it.

**Paper anchor**
- Table I and ablation logic: TargetOnly, ProxyOnly, AnchorOnly, and Proposed/Proposed-CF separate no transfer, unanchored transfer, target-placebo correction, and DR CATE learning.
- Section 3 gives the estimator sequence that makes the ablation interpretation meaningful.

**Design rationale**
- Four cards make the mechanism visible without turning the slide into a table.
- The bottom line now states audience-facing evidence logic: average ranks summarize the pattern after the finite-sample gain is visible.
- Earlier drafts used a "Talk move" callout; that was removed because it read as speaker instruction rather than slide content.

---

## Slide 10 — Robustness: source diversity helps; violations degrade smoothly (11:15–12:30) · Transition in: fade 500 ms

Fade marks the pivot from mechanism to robustness and boundary conditions.

**Paper anchor**
- Source site scaling, Section 4: increasing C from 2 to 50 keeps Proposed around PEHE 0.84 -> 0.66 at m0=150, m1=100 while TargetOnly remains near 5.
- A5 sensitivity: when sparse linear shift holds, Proposed-CF is about 0.38 PEHE; performance degrades as sparsity and nonlinearity increase.

**Design rationale**
- Three cards separate the positive scaling result, robustness result, and limitation.
- Wording avoids process language like "the slide should say"; it states the boundary directly: dense or highly nonlinear corrections reduce performance.
- This slide prevents the results section from sounding like unqualified dominance.
...
- The disconnected card now states "Assumption: A6 transport" so the caveat is content, not presenter instruction.
- The closing line states the final evidentiary boundary: connected estimates identified CATEs; disconnected analyses are transport-based.
...
| m1 | Dual "1.3" callouts ambiguous | Superseded | Results-arc repair no longer opens with the ranking slide, so the dual-callout issue is absent from the current candidate deck. |
